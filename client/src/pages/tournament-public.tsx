                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="bg-white/60 dark:bg-slate-800/60 p-4 rounded-lg border border-blue-200 dark:border-blue-700">
                    ...
                    <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed">
                      ...
                      {categoryFormat === 'league' && 
                        `Na categoria ${categoryName}, a disputa é feita em formato de liga (campeonato de pontos corridos). ${categoryParticipants > 0 ? `Com ${categoryParticipants} participantes, todos` : 'Todos os atletas'} enfrentam todos em turno único ou turno e returno. Cada vitória soma pontos na classificação. Ao final de todas as rodadas, o atleta com mais pontos é declarado campeão. Em caso de empate na liderança, os critérios de desempate incluem: confronto direto, saldo de sets e saldo de pontos. Este formato valoriza a consistência ao longo de toda a competição, recompensando quem mantém bom desempenho em todas as partidas. O vice-campeão é o 2º colocado, e os dois atletas em 3º lugar na classificação final recebem bronze.`}
                      {categoryFormat === 'cup' && (() => {
                        const totalMatches = categoryParticipants > 0 ? categoryParticipants - 1 : 0;
                        return `Na categoria ${categoryName}, a disputa é feita no formato de copa (eliminatória de mata-mata). ${categoryParticipants > 0 ? `Com ${categoryParticipants} participantes, serão realizadas ${totalMatches} partidas eliminatórias.` : ''} Os atletas são distribuídos em um chaveamento único, e cada partida é decisiva: quem vence avança, quem perde está eliminado. O torneio progride por fases (oitavas, quartas, semifinais e final) até definir o campeão. Os dois semifinalistas derrotados disputam o 3º lugar em partida específica. A final define campeão e vice-campeão. É o formato mais emocionante e imprevisível, onde cada partida vale a permanência no torneio.`;
                      })()}
                      {categoryFormat === 'group_stage_knockout' && 
                        `Na categoria ${categoryName}, a disputa é feita em duas fases: fase de grupos e fase eliminatória (modelo Copa do Mundo). Primeira Fase (Grupos): os ${categoryParticipants > 0 ? `${categoryParticipants} participantes são` : 'participantes são'} divididos em grupos. Dentro de cada grupo, todos enfrentam todos em sistema de pontos corridos. Os 2 melhores colocados de cada grupo (considerando vitórias, saldo de sets e critérios de desempate) avançam para a próxima fase. Segunda Fase (Eliminatórias): os classificados entram em chaveamento eliminatório direto. A partir daqui, é mata-mata: quem perde está eliminado. As fases são quartas de final, semifinais e final. Os semifinalistas derrotados ficam com o bronze (3º lugar). A final decide o campeão e vice-campeão. Este formato combina a justiça dos grupos (todos jogam várias partidas) com a emoção da eliminatória direta nas fases finais.`}
                      {categoryFormat === 'team_round_robin' && 
                        `Na categoria ${categoryName}, a disputa é feita por EQUIPES em sistema de pontos corridos. Cada confronto entre duas equipes é chamado de "jogo de equipes", onde vários atletas de cada time enfrentam-se em partidas individuais. O placar de cada jogo de equipes é definido pela soma das vitórias individuais. As equipes somam pontos na classificação geral a cada jogo vencido. Ao final de todas as rodadas (todos contra todos), a equipe com mais pontos é declarada campeã. Em caso de empate, aplicam-se critérios como confronto direto, saldo de jogos e saldo de partidas individuais. Este é o formato clássico de competições por equipes, valorizando desempenho coletivo e estratégia na escalação dos jogadores.`}
                      {categoryFormat === 'team_group_knockout' && 
                        `Na categoria ${categoryName}, a disputa é feita por EQUIPES em duas fases. Primeira Fase (Grupos): as equipes são divididas em grupos e disputam em sistema de pontos corridos. Em cada confronto entre equipes, vários atletas jogam partidas individuais. As equipes com melhor campanha nos grupos avançam. Segunda Fase (Eliminatórias): as equipes classificadas entram em mata-mata por equipes. Cada confronto eliminatório é decidido pelo placar agregado das partidas individuais disputadas. A equipe que vencer o confronto avança, a derrotada é eliminada. As semifinais definem os finalistas e as equipes derrotadas disputam o 3º lugar. A final decide a equipe campeã e vice-campeã. Este formato combina a fase de grupos com a emoção das eliminatórias por equipes.`}
